...
if(isset($_SESSION['isLoggedInToLemonShop']) and $_SESSION['isLoggedInToLemonShop'] == true){
	...
	paypal.Buttons({
    		createOrder: function(data, actions) {
        		return actions.order.create({
            			purchase_units: [{
                			amount: {
                    				currency_code: "USD",
                    				value: "' . $price + 5 . '", // Total value: price of item + shipping
                    				breakdown: {
                        				item_total: {
                            					currency_code: "USD",
                            					value: "' . $price . '" // Price of every item combined
                        				},
                        				shipping: {
                            					currency_code: "USD",
                            					value: "5.00" // Price of shipping
                        				},
                    				}
                			},
                			items: [{
                    				...
                    				unit_amount: {
                         				currency_code: "USD",
                         				value: "' . $price . '" // Price of the item alone
                    				},
                    				quantity: "' . $quantity . '"
                			}]
            			}]
    		...
    		onApprove: function(data, actions) {
        		return actions.order.capture().then(function(details) { // When the transaction succeeds:
